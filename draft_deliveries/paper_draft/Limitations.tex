...
TrimCI-selected determinant space. Although Appendix~\ref{subsec:trimci-validation}
shows that this reduced space reproduces the exact FCI result very well on the
small calibration systems, the larger-system conclusions in this report should
still be interpreted as reduced-space results rather than exact full-space
statements.

A second limitation is that the resource comparison is not yet fully symmetric
between the two basis families. In the current implementation, Krylov basis
...
truncated or selected Krylov construction could make the resource comparison
more balanced against selected UCCSD.

\subsection{Future Work}

One natural extension is to make the hardware-level benchmark more symmetric.
This includes repeating the resource analysis with truncated-Hamiltonian Krylov
constructions and comparing those results directly against warm-start and
selected-UCCSD variants. Such a study would clarify whether lower-cost Krylov
...
based implementations of Krylov time evolution can reduce the resource cost of
the Krylov basis relative to the current Trotterized construction.

A second direction is to expand the choice of reference state beyond
Hartree-Fock. For example, both Krylov and UCCSD could be initialized from a
...
expressivity of Krylov with the shallower state-preparation structure of
selected UCCSD. Another is to investigate related subspace constructions and
variations of UCC/Krylov ideas, for example Q-SENSE
\cite{patel2025qsense}. A complementary theoretical direction is to study more
carefully why Krylov bases so effectively describe the ground state in these
